...
if __name__ == "__main__":
    ...
    for staff_rec in staff_recs:
        if histo[staff_rec.y] > avg :
            staff_recs = staff_rec
        else
    ...
    for box in staff_boxes:
        ...
        while(i < len(staff_notes)):
            if (staff_notes[i].rec.x > staffs[j].x and j < len(staffs)): // 음표의 좌표가 빈오선 보다 큼
                j += 1;
                if staffs[j+1].x - staff[j].x > staff_notes[i].rec.w:
                    k.append(i)
                if len(note_group) > 0: // 그룹 묶음이 끝남
