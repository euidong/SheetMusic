...
if __name__=="__main__":
    filepath='resources/resos'
    for i in range(3,9):
        for j in ['lyric','non-lyric']:
            for k in range(1,9):
                cur_filepath=str(i)+'/'+j+'/'+str(k)+'.jpg'
                cur_filepath=os.path.join(filepath,cur_filepath)
                img = cv2.imread(cur_filepath, 0)
