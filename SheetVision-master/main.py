...
# 이미지를 불러옵니다.
staff_imgs = [cv2.imread(staff_file, cv2.IMREAD_UNCHANGED) for staff_file in staff_files] #오선에 대한 이미집니다.
quarter_imgs = [cv2.imread(quarter_file, cv2.IMREAD_UNCHANGED) for quarter_file in quarter_files]
sharp_imgs = [cv2.imread(sharp_files, cv2.IMREAD_UNCHANGED) for sharp_files in sharp_files] # 샾 이미지
flat_imgs = [cv2.imread(flat_file, cv2.IMREAD_UNCHANGED) for flat_file in flat_files]
half_imgs = [cv2.imread(half_file, cv2.IMREAD_UNCHANGED) for half_file in half_files]
whole_imgs = [cv2.imread(whole_file, cv2.IMREAD_UNCHANGED) for whole_file in whole_files]
...
if __name__ == "__main__":
    ...
    for i in range(len(measures)):
        filename = 'cutting{}.png'.format(i+1)
        cv2.imwrite(filename, measures[i])

    cv2.waitKey(0)
    cv2.destroyAllWindows()

    clef_measure = measures[0]
